...
| [4.2](specs/feature-isbn-lookup/backend/step-4.2-nektar-client.md) | `OszkNektarClient`: YAZ4J Z39.50 kapcsolat, `@attr 1=7` ISBN keresés, marc4j MARC21 parsing, `IsbnSource` enum (`OSZK`, `MANUAL`); egységes `IsbnLookupResponse` DTO a service és controller réteg között; időablak-ellenőrzés nincs (connection timeout fedezi a kiesést) |
| [4.3](specs/feature-isbn-lookup/backend/step-4.3-isbn-lookup-service.md) | `IsbnLookupService`: OSZK lookup → ha `Optional.empty()` → 204 No Content; DEMO rate limit (session: Spring Cache JWT-kötve, napi: DB lazy reset) |
| [4.4](specs/feature-isbn-lookup/backend/step-4.4-isbn-lookup-controller.md) | `IsbnLookupController`: `GET /api/books/isbn/{isbn}` — 200 (találat), 204 (nem található), 422 (érvénytelen ISBN), 429 DEMO rate limit esetén |
